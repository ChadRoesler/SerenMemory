...
seren-memory's own main() calls it before the app is built, and that the
...
OPEN = 'server:\n  host: 0.0.0.0\n  port: 7420\n'
ALLOWED = 'server:\n  host: 0.0.0.0\n  port: 7420\n  allow_open_lan: true\n'
SCRUB = ("SEREN_MEMORY_ALLOW_OPEN_LAN", "SEREN_MEMORY_HOST", "SEREN_MEMORY_PORT", "SEREN_MEMORY_CONFIG",
         "SEREN_MEMORY_BEARER_TOKEN", "SEREN_MEMORY_BEARER_TOKEN_ENV", "SEREN_MEMORY_BEARER_TOKEN_KEYRING",
         "SEREN_MEMORY_TOKEN")
...
def _run(tmp_path, monkeypatch, yaml_text):
    ...
    holder = importlib.import_module("seren_memory.__main__")
    monkeypatch.setattr(holder, "create_app",
                        lambda *a, **k: (_ for _ in ()).throw(_AppBuilt()))
    monkeypatch.setattr(sys, "argv", ["seren_memory", "--config", str(cfg)])
    main_mod = importlib.import_module("seren_memory.__main__")
    main_mod.main()
...
def test_the_env_override_lets_the_same_config_through(tmp_path, monkeypatch):
    ...
    for name in SCRUB:
        monkeypatch.delenv(name, raising=False)
    monkeypatch.setenv("SEREN_MEMORY_ALLOW_OPEN_LAN", "1")
    holder = importlib.import_module("seren_memory.__main__")
    monkeypatch.setattr(holder, "create_app", lambda *a, **k: (_ for _ in ()).throw(_AppBuilt()))
    monkeypatch.setattr(sys, "argv", ["seren_memory", "--config", str(cfg)])
    with pytest.raises(_AppBuilt):
        importlib.import_module("seren_memory.__main__").main()
